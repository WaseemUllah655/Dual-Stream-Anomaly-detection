...
def main():
    ...
    path= r"D:/Violance_datasets/SurveillanceFight42/Train/"
    i3D_model = modelpass()
    ...
    for folder1 in os.listdir(path):
        print("loading folder: " + folder1)
        folder_path = os.path.join(path,folder1)
        
        
        for videos in os.listdir(folder_path):
            ...
            while(i < total_frames - 1):
                ...
                if (ret==1):
                    ...
                    if (len(sequence1) == 16):
                        ...
                        #print(i%10)
                        print(len(sequence1))
                        print(np.array(sequence1).shape)
                        aa = np.asarray(sequence1)
                        # aa = aa.reshape(1, 16, 224, 224, 3).astype('float32')
                        aa = aa.reshape(1, 16, 224, 224, 3)
                        
                        I3D_feature = i3D_model.predict(aa)
                        I3D_feature = I3D_feature.reshape(1024)                        
                        
                        all_data.append(I3D_feature)
                        all_labels.append(folder1)
                        print('*** Single features Shape => ',I3D_feature.shape)
                        print('----- Extracted features shaoe => ',np.array(all_data).shape)
                        print('_______Extracted labels Shape => ',np.array(all_labels).shape)
    ...
    TotalFeatures = np.array(all_data)
    ...
    OneHot=  np.zeros([len(all_data),Classes], dtype='int');
    ...
    np.save('SurveillanceFight42_3dd_features_2classes',TotalFeatures)
    np.save('SurveillanceFight42_3dd_labels_2classes',OneHot)
