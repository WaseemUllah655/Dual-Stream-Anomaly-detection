...
history=model.fit([X_train_OP,X_train_I3D] ,y_train_OP,batch_size=128, epochs=20,validation_data=([X_val_OP,X_val_I3D], y_val_OP))
...
model.save_weights('fine_tune_model_weight_two_steam_ESN')
...
# calculate scores
ns_auc_1 = roc_auc_score(y_val_OP, ns_probs)
lr_auc_1 = roc_auc_score(y_val_OP, lr_probs)
# summarize scores
...
import matplotlib.pyplot as plt
import itertools
confusion = confusion_matrix(np.argmax(y_val_OP,axis=1), np.argmax(y_pred,axis=1))
...
y_pred = (pred > 0.5)*1 
confusion_matrix1=confusion_matrix(y_val_OP.argmax(axis=1), y_pred.argmax(axis=1))
