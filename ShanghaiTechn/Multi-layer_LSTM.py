...
batch_size = 128
epochs = 20
X_vgg = np.vstack(Train_Shanghaitech_refine_features_efn_2_classes_b7)
X_test = np.vstack(Test_Shanghaitech_refine_features_efn_2_classes_b7)


y_train = TestLabels
y_test =TestLabels_000

train_features_final_15frames = np.vstack(Train_Shanghaitech_refine_3dd_features_2classes)
test_features_final=np.vstack(Test_Shanghaitech_refine_3dd_features_2classes)
...
y_train_pf= Train_Shanghaitech_refine_3dd_labels_2classes
y_test_pf =Test_Shanghaitech_refine_3dd_labels_2classes
...
x1 = LSTM(512, return_sequences=True, name = 'LSTM_1')(inputA)
x1 = LSTM(512, return_sequences=False, name = 'LSTM_2')(x1)
#
...
history=model.fit([X_train_OP,X_train_I3D] ,y_train_OP,batch_size=batch_size, epochs=100,validation_data=([X_val_OP,X_val_I3D], y_val_OP))
# summarize layers

# plot graph
#plot_model(model, to_file='multiple_inputs.png')
model.save('F:\Waseem_Data\Two_CNN_stream\ShanghaiTechn\Result of ShanghaiTech\LSTM\Models/fine_tune_model_two_steam_LSTM.h5')
model.save_weights('F:\Waseem_Data\Two_CNN_stream\ShanghaiTechn\Result of ShanghaiTech\LSTM\Models/fine_tune_model_weight_two_steam_LSTM')
...
#
y_pred = (pred > 0.5)*1 
confusion = confusion_matrix(y_val_OP.argmax(axis=1), y_pred.argmax(axis=1))
...
model.save('F:\Waseem_Data\Two_CNN_stream\ShanghaiTechn\Result of ShanghaiTech\LSTM\Models\Two-stream-CNN_LSTM.h5')
score, acc = model.evaluate([X_val_OP,X_val_I3D], y_val_OP, batch_size=100)
#score, acc = model.evaluate([X_test,test_features_final],y_test_pf, batch_size=batch_size)
...
print(classification_report(np.argmax(y_val_OP,axis=1), np.argmax(y_pred,axis=1), target_names=target_names))
